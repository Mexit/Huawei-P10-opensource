...
int unpack_hevc_ps(AVCodecContext *codec, AVPacket *pkt, int* find_hevc_ps)
{
    int i, iOffset=0;

    int iStreamLen = pkt->size;

    int extradata_size = codec->extradata_size;

    int ret = *find_hevc_ps, iFlag=0;
    uint8_t *ptr = NULL, *pExtradataTmp=NULL, *pStream = pkt->data;
    ...
    state4=(pStream[1] <<24) | (pStream[2] <<16) | (pStream[3] <<8) | pStream[4];

    if (  (state4&0xFFFFFF7E) == 0x140
        ||(state4&0xFFFFFF7E) == 0x142
        ||(state4&0xFFFFFF7E) == 0x144)
    {
        if (codec->extradata == NULL)
        {
            iOffset = 0;

            codec->extradata      = av_malloc(iStreamLen+5);
            codec->extradata_size = iStreamLen+5;
        }
        else
        {
             pExtradataTmp = av_realloc(codec->extradata, iStreamLen+5+codec->extradata_size);
             iOffset = codec->extradata_size;

             codec->extradata = pExtradataTmp;
             codec->extradata_size += iStreamLen+5;
        }

        ptr = codec->extradata;
        ptr[22]++;
        ptr[iOffset] = (pStream[4]>>1)|0b10000000;
        ptr[iOffset+1] = 0;
        ptr[iOffset+2] = 1;
        ptr[iOffset+3] = (iStreamLen>>8) & 0b11111111;
        ptr[iOffset+4] = (iStreamLen) & 0b11111111;

        memcpy(ptr+iOffset+5, pStream, iStreamLen);
        iFlag = (pStream[4] & 0x0F)<<1;
        iFlag = (iFlag > 0) ? iFlag : 2;
...
int k3_find_hevc_ps(AVFormatContext *ic, AVPacket *pkt, int* find_hevc_ps)
{
    AVCodecContext *codec = ic->streams[pkt->stream_index]->codec;
    int i, iStart=0;

    int iStreamLen = pkt->size;
    uint8_t* pStream = pkt->data, *pExtradataTmp;

    int extradata_size = codec->extradata_size;
    uint8_t *pExtradata= codec->extradata;

    static const uint8_t kNALStartCode[4] =
               { 0x00, 0x00, 0x00, 0x01 };

    int ret = *find_hevc_ps;
    uint8_t *ptr;
    ...
    if (ret == 0)
    {
        if (NULL != codec->extradata)
        {
            pktTmp.size = codec->extradata_size;
            pktTmp.data = av_malloc(codec->extradata_size);
            memcpy(pktTmp.data, codec->extradata, codec->extradata_size);

//            pExtradataTmp = av_realloc(codec->extradata, codec->extradata_size+18);

//            codec->extradata = pExtradataTmp;
//            codec->extradata_size = 18;
...
//        else
//        {
            codec->extradata_size = 23;
            codec->extradata = av_malloc(codec->extradata_size);
//        }
        ptr = (uint8_t *)codec->extradata;

        ptr[0] = 1;
        ptr[10] = 0b11111100; //reserved 111111 + parallismType 2 bits
        ptr[11] = 0b11111101; //reserved 111111 + chromaFormat 2 bits->01: 4:2:0
        ptr[12] = 0b11111000; //reserved 11111 + bitDepthLumaMinus8 3bits
        ptr[13] = 0b11111000; //reserved 11111 + bitDepthChromaMinus8 3 bits
        ptr[21] = 0xff;       //lengthSizeMinusOne
        ptr[22] = 0;          //numOfArrays

        ret = 1;

        ret = unpack_hevc_ps(codec, &pktTmp, &ret);
    }

    ret = unpack_hevc_ps(codec, pkt, &ret);

    if (NULL != pktTmp.data)
    {
        av_freep(&pktTmp.data);
    }
    return ret;
}

int k3_find_stream_info(AVFormatContext *ic, int tryDecodeNumber)
{
    AVDictionary **options = NULL;
    int i, count, ret = 0, j, k3_count = 0;
    int find_hevc_ps = 0;
    int64_t read_size;
    AVStream *st;
    AVPacket pkt1, *pkt;
    ...
    if (ic->nb_streams > 128) {
        av_log(ic, AV_LOG_ERROR, "ic->nb_streams is %u exceed maximum 128,so this is a abnormal stream,we cann't support it !!!\n", ic->nb_streams);
        return -1;
    }
...
/*            if (!st->time_base.num)
    ...
    count     = 0;
    read_size = 0;
    tryDecodeNumber = tryDecodeNumber < 0 ? 100000 : tryDecodeNumber;
    for (k3_count = 0; k3_count < tryDecodeNumber; k3_count++) {
    //for(;;) {
        ...
        if (AV_CODEC_ID_HEVC == ic->streams[pkt1.stream_index]->codec->codec_id)
        {
            if ( 0xF != find_hevc_ps)
            {
                find_hevc_ps = k3_find_hevc_ps(ic, &pkt1, &find_hevc_ps);
            }
        }
...
//4 AVCodecIDs per line, pay attention to the use of \ to change line
#define QCOM_MIN_SET AV_CODEC_ID_H263, AV_CODEC_ID_H263P, AV_CODEC_ID_H263I, AV_CODEC_ID_H264,\
                     AV_CODEC_ID_MPEG4, AV_CODEC_ID_MSMPEG4V1, AV_CODEC_ID_MSMPEG4V2, AV_CODEC_ID_MSMPEG4V3,\
                     AV_CODEC_ID_MPEG2VIDEO, AV_CODEC_ID_H265, AV_CODEC_ID_VP8, AV_CODEC_ID_HEVC,\
                     AV_CODEC_ID_MP3, AV_CODEC_ID_AAC, AV_CODEC_ID_VORBIS, AV_CODEC_ID_MP2,\
                     AV_CODEC_ID_FLAC

#define MTK_MIN_SET  AV_CODEC_ID_H263, AV_CODEC_ID_H263P, AV_CODEC_ID_H263I, AV_CODEC_ID_H264,\
                     AV_CODEC_ID_MPEG4, AV_CODEC_ID_MSMPEG4V1, AV_CODEC_ID_MSMPEG4V2, AV_CODEC_ID_MSMPEG4V3,\
                     AV_CODEC_ID_MPEG2VIDEO, AV_CODEC_ID_H265, AV_CODEC_ID_VP8, AV_CODEC_ID_HEVC,\
                     AV_CODEC_ID_MP3, AV_CODEC_ID_AAC, AV_CODEC_ID_VORBIS, AV_CODEC_ID_MP2,\
                     AV_CODEC_ID_FLAC

//Minimum set of platform
static const enum AVCodecID kQcom_min_set[] = {QCOM_MIN_SET};
static const enum AVCodecID kMtk_min_set[] = {MTK_MIN_SET};

//Product decoder customization structure
typedef struct {
    const char *product_name;
    const enum AVCodecID *product_cust_set;
    const int cust_codec_number;
} product_cust;

//Qualcomm platform product custom configuration decoder list
static const product_cust kQcom_product_cust_list[] = {

    /*
     * product_name : Can be a regular expression
     * Examples
     * "^MLA$" => A Direct Match to MLA
     * "MLA-AL..$" => Starting with MLA-AL and ends with 2 chars. MLA-AL00 is ok. MLA-AL001 not ok.
     * "MLA-AL..$|CAZ-TL..$" => Can use pipe (|) to add multiple expressions.
     *
     */

    {
        "productA", //product name from ro.product.name,need to modify for custom configuration
        (const enum AVCodecID [])
        {QCOM_MIN_SET}, //custom_codec_set:contains all the codecID product required open,need to modify for custom configuration
        sizeof(kQcom_min_set)/sizeof(kQcom_min_set[0]) //codec number of custom_codec_set,need to modify for custom configuration
    },
    {
        "productB",
        (const enum AVCodecID [])
        {QCOM_MIN_SET, AV_CODEC_ID_H263},
        sizeof(kQcom_min_set)/sizeof(kQcom_min_set[0]) + 1
    },
    {   "productC",
        (const enum AVCodecID [])
    ...
    {   "^MLA$|MLA-AL..$|MLA-UL..$|MLA-TL..$|MLA-L..$|^CAZ$|CAZ-TL..$|CAZ-AL..$|^CAN$|CAN-L..$",
        (const enum AVCodecID [])
        {QCOM_MIN_SET, AV_CODEC_ID_MPEG1VIDEO,AV_CODEC_ID_MJPEG,AV_CODEC_ID_APE,AV_CODEC_ID_WMAV2},
        sizeof(kQcom_min_set)/sizeof(kQcom_min_set[0]) + 4
    },
};

//MTK platform product custom configuration decoder list
static const product_cust kMtk_product_cust_list[] = {
    {
        "productA", //product name from ro.product.name,need to modify for custom configuration
        (const enum AVCodecID [])
        {MTK_MIN_SET}, //custom_codec_set:contains all the codecID product required open,need to modify for custom configuration
        sizeof(kMtk_min_set)/sizeof(kMtk_min_set[0]) //codec number of custom_codec_set,need to modify for custom configuration
    },
    {
        "productB",
        (const enum AVCodecID [])
        {MTK_MIN_SET, AV_CODEC_ID_VORBIS},
        sizeof(kMtk_min_set)/sizeof(kMtk_min_set[0]) + 1
    },
    {   "productC",
        (const enum AVCodecID [])
        {AV_CODEC_ID_H263, AV_CODEC_ID_H263P, AV_CODEC_ID_H263I},
...
typedef struct {
    ...
    const product_cust *product_cust_list;
    const int product_cust_list_size;
    const enum AVCodecID *min_set;
    const int min_set_number;
} platform;

//Platform list
static const platform kPlatform_list[] = {
    {   "msm",
        kQcom_product_cust_list,
        sizeof(kQcom_product_cust_list)/sizeof(kQcom_product_cust_list[0]),
        kQcom_min_set,
        sizeof(kQcom_min_set)/sizeof(kQcom_min_set[0]),
    },
    {   "mtk",
        kMtk_product_cust_list,
        sizeof(kMtk_product_cust_list)/sizeof(kMtk_product_cust_list[0]),
        kMtk_min_set,
        sizeof(kMtk_min_set)/sizeof(kMtk_min_set[0]),
    }
};

// Regular expression Matching
static int reg_exp_match(const char *pattern, const char *str)
{
    int    status;
    regex_t    re;
...
//return whether the decoder codecID is customized support: support return 1, unsupport return 0
static int av_is_codecid_supported(enum AVCodecID codec_id) {
    char prop_platform[PROPERTY_VALUE_MAX];
    char prop_product[PROPERTY_VALUE_MAX];
    int i, j, k;

    if (!property_get("ro.board.platform", prop_platform, "unknown")) {
        return 0;
    }

    for (i = 0; i < sizeof(kPlatform_list)/sizeof(kPlatform_list[0]); i++) {
        if (!strncmp(prop_platform, kPlatform_list[i].platform_name, 3)) {
            if (property_get("ro.product.name", prop_product, "unknown")) {
                for (j=0; j < kPlatform_list[i].product_cust_list_size; j++) {
                    if(reg_exp_match(kPlatform_list[i].product_cust_list[j].product_name, prop_product)) {
                        for (k=0; k < kPlatform_list[i].product_cust_list[j].cust_codec_number; k++) {
                            if (codec_id == kPlatform_list[i].product_cust_list[j].product_cust_set[k]) {
                                return 1;
                            }
                        }
                        return 0;
            ...
            for (j = 0; j < kPlatform_list[i].min_set_number; j++) {
                if (codec_id == kPlatform_list[i].min_set[j]) {
                    return 1;
                }
            }
                        ...
                        AVCodec **decoder_ret, int flags)
{
    ...
    for (i = 0; i < nb_streams; i++) {
        ...
        if (!av_is_codecid_supported(avctx->codec_id))
            continue;

        if((avctx->codec_id == AV_CODEC_ID_AC3)
            || (avctx->codec_id == AV_CODEC_ID_EAC3)
            || (avctx->codec_id == AV_CODEC_ID_DTS)
            || (avctx->codec_id == AV_CODEC_ID_RAWVIDEO)) {
                av_log(ic, AV_LOG_ERROR, "We can't support this codecID = 0x%08x", avctx->codec_id);
                continue;
            }
